export const COOKIE_PAYLOAD_TOKEN='payload-token'


export const GENERAL_ERROR_MESSAGE="Có lỗi xảy ra vui lòng thử lại sau"
export const EXCESS_QUANTITY_OPTION_MESSAGE="Nếu bạn muốn mua > 15kg vui lòng liên hệ qua Zalo"

export const INVALID_TOKEN_OR_EXPIRED_MESSAGE= "Mã OTP đã hết hạn hoặc sai vui lòng nhập lại."
export const INVALID_TOKEN_MESSAGE= "Mã OTP không đúng"
export const NOT_FOUND_USER_WITH_THIS_TOKEN_MESSAGE="Không có người dùng nào với tài khoản này"
export const AMOUNT_PER_ADJUST_QUANTITY=0.5

export const MAXIMUN_KG_CAN_BUY_THROUGH_WEB=15

export const EXPIRATION_TIME_OTP=60
export const JWT_EXPIRATION_TIME='30 days'
export const COOKIE_USER_PHONE_NUMBER_TOKEN='user-token'